...
import { getRollTier } from "../rolls/quest-roll.js";
...
export default class QuestCharacterSheet extends foundry.applications.api.HandlebarsApplicationMixin(
...
_initApRightClick() {
  const badges = this.element.querySelectorAll(".ap-badge");
  for (const badge of badges) {
    badge.addEventListener("contextmenu", (event) => {
      event.preventDefault();
      const cost = Number(badge.dataset.cost) || 0;
      const current = this.actor.system.ap;
      this.actor.update({ "system.ap": current + cost });
    });
  }
}
...
_initItemDragStart() {
  ...
  _initDropZones() {
    this.element.querySelectorAll(".abilities-tab, .inventory-tab").forEach(zone => {
...
async _postAbilityToChat(itemId) {
  ...
  /** Currently active tab, tracked in memory per sheet instance. */  
...
const abilityItems = this.actor.items
  ...
  .map(item => ({
    ...
    renderedDescription: renderAbilityDescription(item.system.description),
    pathOrder: item.system.pathOrder ?? 0,
...
const pathGroups = {};
    for (const ability of abilityItems) {
      const pathName = ability.system.learningPath || "";
      if (!pathGroups[pathName]) {
        pathGroups[pathName] = [];
      }
      pathGroups[pathName].push(ability);
    }

    context.abilityPaths = Object.entries(pathGroups).map(([pathName, abilities]) => ({
      pathName,
      abilities: abilities.sort((a, b) => a.pathOrder - b.pathOrder)
    }));
    ...
    for (let i = 0; i < slotCount; i++) {
      ...
      if (found) {
        slots.push({
          ...
          renderedDescription: renderItemDescription(found.system.description),
          slotNumber: i + 1,
  ...
  _getTabs() {
    const tabs = {
      ...
      abilities: { id: "abilities", label: "Abilities" },
  ...
  static async _onSpendAp(event, target) {
    const cost = Number(target.dataset.cost) || 0;
    const current = this.actor.system.ap;
    await this.actor.update({ "system.ap": current - cost });
  }  
}